...
class Ionosphere < Sinatra::Base
  include Transmitter
  
  helpers Sinatra::Param, Sinatra::InvoiceHelpers, Sinatra::OrderHelpers
  
  configure do
    set :raise_errors, false
    set :show_exceptions, :after_handler
    set server: 'thin', subscriptions: []
  ...
  get '/subscribe' do
    ...
    stream :keep_open do |out|
      settings.subscriptions << out
      out.callback { settings.subscriptions.delete(out) }
      # purge dead subscription connections
      settings.subscriptions.reject!(&:closed?)
    end
  end
  ...
  def start_transmitter
    op = Proc.new do
      sent_message = transmit_next_message

      # ugly hack to prevent DB connection exhaustion; thread is about to terminate, so give back the ActiveRecord connection
      ActiveRecord::Base.clear_active_connections!

      sent_message
    end
    
    callback = Proc.new do |sent_order|
      sent_order.update(:status => :sent, :upload_ended_at => Time.now)
      settings.subscriptions.each { |out| out << order_status_sse(sent_order) }
  ...
  def start_stayin_alive
    EM::PeriodicTimer.new(20) {
      settings.subscriptions.each { |out| out << "\0" }
    }
  end
